...
def plot_ndarray(Bkg: np.ndarray, Vmin: int, Vmax: int, mapping: str = 'viridis'):

    plt.figure(figsize=(10, 8))
    # Create the plot
    im = plt.imshow(Bkg, cmap=mapping, aspect='equal',
...
def plot_azimuthal_average(radii, intensities, pixel_to_q: int = 0.024):
    ...
    plt.plot(q_range, intensities, color= 'SlateGray', label='Azimuthal Average')
